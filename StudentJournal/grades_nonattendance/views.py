        ...
        if grades_nonatt_is_valid(post_data):
            for grade_nonatt in post_data.items():
                ...
                if grade_nonatt[1] == "DE":
                    ...
                    LessonResults.objects.update_or_create(student=student, lesson=lesson, grade=grade)
                else:
                    non_attendance_reason = grade_nonatt[1]
                    LessonResults.objects.update_or_create(student=student, lesson=lesson, non_attendance_reason=non_attendance_reason)
...
def get_nonatt_report(request, student_id, term):
    term_start_date = QuarterSchedule[int(term - 1)]["start_date"]
    term_end_date = QuarterSchedule[int(term - 1)]["end_date"] + datetime.timedelta(days=1)

    student = get_object_or_404(AppUser, pk=student_id)

    nonatt_records = LessonResults.objects.filter(
        Q(student = student) & 
        Q(lesson__lesson_holding_time_start__gte = term_start_date) & 
        Q(lesson__lesson_holding_time_start__lte = term_end_date) &
        Q(non_attendance_reason__isnull = False)
    )


def get_grades_report(request, student_id, term):
    pass
...
def get_grades_nonatt_as_dict(students_records, term: int) -> dict:
    ...
    res.update(
        {
            f"grade-nonatt-{grade.student.id}-{grade.lesson.id}": grade.grade for grade in grades
        }
    )
